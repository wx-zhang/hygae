...
try:
    from verl.utils.dataset.rl_dataset import collate_fn, process_image
except ImportError:
...
class QwenVLRolloutManager:
    def __init__(
        ...
        self.global_env_counter = 0  # Global counter for unique environment IDs

    @torch.no_grad()
    def _handle_special_tokens(
        self, llm_raw_response: str, prep_for_loss_mask: bool
    ) -> str:
        """
    ...
    def _handle_multi_modal_data(
        ...
        """
        assert len(image_data) == prompt_template.count(
            "<image>"
        ), "Number of images does not match number of <image> in the prompt template"
        raw_prompt = prompt_template.replace(
    ...
    @torch.no_grad()
    def _compute_think_mask(self, input_ids, loss_mask):
        """Mark generated tokens inside complete ``<think>...</think>`` spans."""
        ...
        think_mask = torch.zeros_like(loss_mask)

        def find_positions(ids, pattern, end=False):
            if len(pattern) == 1:
                return (ids == pattern[0]).nonzero(as_tuple=True)[0].tolist()
            pattern_tensor = torch.tensor(pattern, dtype=ids.dtype, device=ids.device)
            positions = []
            for position in range(sequence_length - len(pattern) + 1):
                if torch.equal(ids[position : position + len(pattern)], pattern_tensor):
                    positions.append(position + len(pattern) - 1 if end else position)
            return positions

        for batch_idx in range(batch_size):
            starts = find_positions(input_ids[batch_idx], start_ids)
            ends = find_positions(input_ids[batch_idx], end_ids, end=True)
            end_idx = 0
            for start in starts:
                ...
                if end_idx >= len(ends):
                    break
                think_mask[batch_idx, start : ends[end_idx] + 1] = 1
                end_idx += 1
        return think_mask * loss_mask
    ...
    def _compute_loss_mask(self, input_ids, attention_mask):
        ...
        sptk_b = self.tokenizer.convert_tokens_to_ids(
            self.config.special_token_for_loss_mask[0]
        )
        sptk_e = self.tokenizer.convert_tokens_to_ids(
            self.config.special_token_for_loss_mask[1]
        )
        ...
        for b in range(batch_size):
            ...
            if not torch.all(attention_mask[b, -right_pad_tokens:] == 0):
                print("Warning: right padding tokens must have attention mask 0")
            ...
            hole_pos = (
                []
            )  # initialize holes position list with last padding token position
            ...
            if not torch.all(
                new_input_ids[b][seq_len - right_pad_tokens :] == pad_token_id
            ):
                print("Warning: right padding tokens must equal the pad token")
            ...
            for i in range(0, len(hole_pos) - 1):
                ...
                new_loss_mask[
                    b, start_pos + 1 - holes_to_fill : end_pos - holes_to_fill
                ] = loss_mask[b, start_pos + 1 : end_pos]
                new_end_of_response_position_mask[
                    b, start_pos + 1 - holes_to_fill : end_pos - holes_to_fill
                ] = end_of_response_position_mask[b, start_pos + 1 : end_pos]
                new_input_ids[
                    b, start_pos + 1 - holes_to_fill : end_pos - holes_to_fill
                ] = input_ids[b, start_pos + 1 : end_pos]
                new_attention_mask[
                    b, start_pos + 1 - holes_to_fill : end_pos - holes_to_fill
                ] = attention_mask[b, start_pos + 1 : end_pos]
                holes_to_fill += 1

            valid_tokens = (
                seq_len - right_pad_tokens - len(hole_pos) + 1
            )  # the number of non-special tokens and non-padding tokens
    ...
    def reset(self, env_configs):
        ...
        self.env_states = {
            env_id: {
                "step": 0,
                "done": False,
                "trajectory_id": self.global_env_counter
                + env_id,  # Global unique ID for this trajectory
                "metrics": {"turn_metrics": defaultdict(list), "traj_metrics": {}},
            }
            for env_id in self.envs
        }
        # Increment global counter by the number of environments in this batch
        self.global_env_counter += len(self.envs)
    ...
    def _generate_input_for_rollout(
        ...
        if has_images:  # expand image token
            prompt_with_chat_template, row_dict, _, raw_prompt = (
    ...
    def _generate_input_for_uptate(
        ...
        row_dict = {}
        if has_images:  # expand image token
            response_with_chat_template, row_dict, image_grid_thw, _ = (
                self._handle_multi_modal_data(
                    response_with_chat_template, row_dict, image_data, do_embedding=True
                )
            )
        row_dict["chat"] = response_rst["chat"]
        ...
        input_ids_response, attention_mask_response = (
            verl_F.tokenize_and_postprocess_data(
                ...
                - 1,  # -1 for the prompt padding token
                pad_token_id=self.tokenizer.pad_token_id,
                left_pad=False,
        ...
        attention_mask_prompt = torch.zeros_like(
            input_ids_prompt
        )  # All prompt will be masked
        ...
        end_of_response_position_mask_response = end_of_response_position_mask_response[
            0
        ]

        loss_mask = torch.cat([loss_mask_prompt, loss_mask_response], dim=-1)
        end_of_response_position_mask = torch.cat(
            [
                end_of_response_position_mask_prompt,
                end_of_response_position_mask_response,
            ],
            dim=-1,
        )
        ...
        attention_mask = torch.cat(
            [attention_mask_prompt, attention_mask_response], dim=-1
        )
        ...
        if has_images:
            ...
            delta_position_id = torch.arange(
                1, response_length + 1, device=position_ids_prompt.device
            )
        ...
        if self.config.use_multi_turn_reward:
            ...
            multi_turn_token_level_rewards = torch.zeros_like(
                end_of_response_position_mask, dtype=torch.float
            )
            assert len(reward_positions) == len(
                rewards
            ), "Number of rewards does not match number of reward positions"
            for idx, reward in enumerate(rewards):
                multi_turn_token_level_rewards[reward_positions[idx]] = reward
            row_dict["multi_turn_token_level_rewards"] = (
                multi_turn_token_level_rewards  # (seq_len,)
            )
        ...
        row_dict["think_mask"] = self._compute_think_mask(
            input_ids.unsqueeze(0), loss_mask.unsqueeze(0)
        ).squeeze(0)
        row_dict["end_of_response_position_mask"] = end_of_response_position_mask  #
    ...
    def generate_batch_for_rollout(self, step, window_size):
        ...
        for env_id in self.envs.keys():
            ...
            batch.append(
                self._generate_input_for_rollout(
                    self.recorder[env_id], step, window_size
                )
            )
    ...
    def _log_data_to_local(
        ...
        """
        Save debug data for a single step: inputs, outputs, images, and conversation history.
        ...
        Args:
            ...
            output_batch: Output batch from model
        ...
        # Get debug output directory from config, default to 'debug_output'
        debug_dir = Path("generation_logs")
        # Use unique trajectory_id instead of env_id
        trajectory_id = self.env_states[env_id].get("trajectory_id", env_id)
        trajectory_dir = debug_dir / f"trajectory_{trajectory_id}"
        trajectory_dir.mkdir(parents=True, exist_ok=True)

        # Save input prompt for this step
        batch_idx = None
        for idx, eid in self.batch_idx_to_env_id.items():
            if eid == env_id:
                batch_idx = idx
                break
        ...
        if batch_idx is not None:
            ...
            if "multi_modal_data" in gen_batch.non_tensor_batch:
                try:
                    multi_modal_data = gen_batch.non_tensor_batch["multi_modal_data"]
                    if batch_idx < len(multi_modal_data):
                        modal_data = multi_modal_data[batch_idx]
                        if isinstance(modal_data, dict) and "image" in modal_data:
                            input_images = modal_data["image"]
                            if isinstance(input_images, list):
                                for img_idx, img in enumerate(input_images):
                                    img_path = (
                                        trajectory_dir
                                        / f"step_{step}_input_image_{img_idx}.png"
                                    )
                                    if isinstance(img, PIL.Image.Image):
                                        img.save(img_path)
                except (IndexError, KeyError, TypeError) as e:
                    # Silently skip if there's an issue accessing images
                    pass

            # Decode input prompt if available
            if "raw_prompt_ids" in gen_batch.non_tensor_batch:
                try:
                    ...
                    elif not isinstance(raw_prompt_ids, list):
                        raw_prompt_ids = list(raw_prompt_ids)
                    input_prompt = self.tokenizer.decode(
                        raw_prompt_ids, skip_special_tokens=False
                    )
                    input_file = trajectory_dir / f"step_{step}_input_prompt.txt"
                    with open(input_file, "w", encoding="utf-8") as f:
                        f.write(f"Step {step} Input Prompt:\n")
                        f.write("=" * 80 + "\n")
                        f.write(input_prompt)
                        f.write("\n")
                except (IndexError, KeyError, TypeError) as e:
                    # Silently skip if there's an issue decoding prompt
                    pass

            # Save output response
            if batch_idx < len(responses_str):
                response = responses_str[batch_idx]
                output_file = trajectory_dir / f"step_{step}_output_response.txt"
                with open(output_file, "w", encoding="utf-8") as f:
                    f.write(f"Step {step} Output Response:\n")
                    f.write("=" * 80 + "\n")
                    f.write(response)
                    f.write("\n")
                    f.write(f"\nReward: {reward}\n")
    ...
    def rollout_loop(self):
        ...
        for step in range(self.config.max_turns):
            ...
            raw_prompt_ids = gen_batch.non_tensor_batch["raw_prompt_ids"]
            raw_prompt_ids_array = np.ndarray(
                shape=(len(raw_prompt_ids),), dtype=object
            )
            ...
            for batch_idx, env_id in self.batch_idx_to_env_id.items():
                obs, reward, done, info = self.envs[env_id].step(
                    responses_str[batch_idx]
                )
                self.env_states[env_id]["step"] += 1
                self.env_states[env_id]["done"] = done
                self.env_states[env_id]["metrics"]["traj_metrics"] = info[
                    "metrics"
                ].get("traj_metrics", {})
                ...
                # Save data to local file if local_log is enabled
                if self.config.local_log:
                    self._log_data_to_local(
                        env_id,
                        step,
                        input_batch_dict,
                        gen_batch,
    ...
    def generate_batch_for_update(self) -> DataProto:
        ...
        for env_id in self.envs.keys():
            ...
            row_dict["reward_model"] = {
                "style": "given",
                "ground_truth": {"reward": last_reward + step_reward_sum},
            }
            if self.config.use_multi_turn_reward:
                end_of_response_position_mask = row_dict[
                    "end_of_response_position_mask"
                ]
                reward_positions = torch.nonzero(end_of_response_position_mask).squeeze(
                    -1
                )
    ...
    def recording_to_log(self):
        ...
        for env_id, record in self.recorder.items():
            ...
            step = self.env_states[env_id]["step"]
            is_final_log = self.env_states[env_id]["done"] or (
                self.env_states[env_id]["step"] >= self.config.max_turns
            )
