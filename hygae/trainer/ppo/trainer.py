...
from hygae.rollout.qwen_rollout.rollout_manager import QwenVLRolloutManager
...
from hygae.trainer.ppo.advantage_estimator import *
...
def apply_kl_penalty(
    ...
    eos_mask = data.batch["loss_mask"][:, -response_length:]
    ...
    if reduce == "turn":
        reward_masks = data.batch["end_of_response_position_mask"][
            :, -response_length:
        ]  # (bs, L)
        for b in range(batch_size):
            turn_pos = reward_masks[b].nonzero(as_tuple=True)[0]
            if len(turn_pos) == 0:
                continue
            last_idx = 0
            for idx in turn_pos:
                turn_mask = torch.zeros_like(
                    response_mask[b], dtype=torch.bool)
                turn_mask[last_idx: idx + 1] = True
                last_idx = idx + 1
                turn_mask = turn_mask * eos_mask[b]
                mean_kld = masked_mean(
                    kld[b: b + 1], mask=turn_mask.unsqueeze(0), axis=-1
                ).item()

                kld[b][turn_mask.bool()] = mean_kld
...
def _compute_response_info(batch):
    if "loss_mask" in batch.batch.keys():
        ...
        prompt_length = (
            batch.batch["attention_mask"].sum(-1) -
            batch.batch["loss_mask"].sum(-1)
        ).float()
...
class Trainer(object):
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator in [
            ...
            AdvantageEstimator.BI_LEVEL_GAE,
            AdvantageEstimator.MASKED_GAE,
            AdvantageEstimator.TURN_WISE_GAE,
            AdvantageEstimator.HYGAE,
            AdvantageEstimator.RL4VLM,
        ]:
            self.use_critic = True
        elif self.config.algorithm.adv_estimator in [
            AdvantageEstimator.GRPO,
            AdvantageEstimator.MASKED_GRPO,
            AdvantageEstimator.REINFORCE_PLUS_PLUS,
            AdvantageEstimator.MASKED_REINFORCE,
            AdvantageEstimator.REMAX,
            AdvantageEstimator.RLOO,
            AdvantageEstimator.MULTI_TURN_GRPO,
        ]:
    ...
    def _validate_config(self):
        ...
        if self.use_critic and not config.critic.use_dynamic_bsz:
            ...
            if config.algorithm.adv_estimator == AdvantageEstimator.TURN_WISE_GAE:
                assert config.critic.get(
                    "use_reward_mask", False
    ...
    def _create_dataloader(self):
        ...
        try:
            # New verl API
            train_dataset_config = OmegaConf.create({
        ...
        try:
            val_dataset_config = OmegaConf.create({
                "prompt_key": self.config.data.prompt_key,
                "image_key": self.config.data.get("image_key", "images"),
                "max_prompt_length": self.config.data.max_prompt_length,
                "filter_prompts": True,
    ...
    def _maybe_log_val_generations_to_wandb(self, log_rst: List[Dict[str, Any]]):
        ...
        max_N = generations_to_log

        if has_images:
            global_max_images = 1
            for img_data in images:
                if isinstance(img_data, (list, tuple)):
                    global_max_images = max(global_max_images, len(img_data))
                elif img_data is not None:
                    global_max_images = max(global_max_images, 1)
            max_images_per_sample = global_max_images
        else:
        ...
        if has_images:
            ...
            for i in range(max_N):
                ...
                columns.extend(
                    [f"image_{i+1}_{j+1}" for j in range(
        ...
        if not hasattr(self, "validation_table"):
            ...
            self._val_table_max_images_per_sample = max_images_per_sample
            self._val_table_max_samples = max_N
        else:
            columns = self.validation_table.columns
            max_images_per_sample = getattr(
                self, "_val_table_max_images_per_sample", max_images_per_sample
            )
            max_N = getattr(self, "_val_table_max_samples", max_N)

        row_data = [self.global_steps]

        padded_samples = list(samples) + [None] * (max_N - len(samples))

        for i in range(max_N):
            sample = padded_samples[i]
            ...
            if has_images:
                ...
                wandb_images.extend(
                    [None] * (max_images_per_sample - len(wandb_images))
                )
        ...
        wandb.log({"val/generations": self.validation_table},
    ...
    def _validate(self):
        ...
        for batch_dict in self.val_dataloader:
            ...
            env_configs = [
                batch.non_tensor_batch["extra_info"][i] for i in range(len(batch))
            ]
            self.test_rollout_manager.reset(env_configs)
            self.test_rollout_manager.rollout_loop()
            micro_validation_rst = (
                self.test_rollout_manager.recording_to_log()
            )  # data source == inputs in our current setting, outputs=whole trjecotry
    ...
    def init_workers(self):
        ...
        # Create a modified copy of actor_rollout_ref config with kl_reduce removed from actor
        # This is needed because FSDPActorConfig doesn't accept kl_reduce parameter
        # but we still need it accessible for apply_kl_penalty function
        actor_rollout_ref_config = deepcopy(self.config.actor_rollout_ref)
        with open_dict(actor_rollout_ref_config.actor):
            if "kl_reduce" in actor_rollout_ref_config.actor:
                # Store kl_reduce value for later use, then remove it from config
                # The original config still has it, so apply_kl_penalty can access it
                del actor_rollout_ref_config.actor.kl_reduce
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(dataloader_local_path):
            # This is a trusted, locally generated trainer checkpoint. Keep the
            # historical load semantics explicitly under PyTorch 2.6, whose
    ...
    def _process_in_mini_batches(self, batch, rollout_manager, mini_batch_size):
        ...
        ) // mini_batch_size  # Ceiling division
        ...
        for i in range(num_mini_batches):
            ...
            print(
                f"Processing mini-batch {i+1}/{num_mini_batches}, size: {actual_mini_batch_size}"
            )
            ...
            # Extend the list since rst is a list
            all_rst.extend(mini_batch_rst)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                batch = batch.repeat(
                    repeat_times=self.config.rollout_manager.n_trajectory,
                    interleave=True,
                )
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        mini_batch_size = self.config.rollout_manager.get(
                            "mini_batch_size", len(batch)
                        )
                        final_gen_batch_output, rst = self._process_in_mini_batches(
                            batch, rollout_manager, mini_batch_size
                        )
                        train_metrics = self.log_rst_to_metrics_dict(
                            rst=rst, mode="train"
                        )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.rollout_manager.use_multi_turn_reward:
                            ...
                            batch.batch["token_level_scores"] = batch.batch[
                                "multi_turn_token_level_rewards"
                            ][:, -response_len:]
                        ...
                        if not self.config.actor_rollout_ref.actor.get(
                            ...
                            batch, kl_metrics = apply_kl_penalty(
                                ...
                                reduce=self.config.actor_rollout_ref.actor.get(
                                    "kl_reduce", None
                                ),
                            )
                        ...
                        batch = compute_advantage(
                            ...
                            aggregate_reward_in_turn=self.config.algorithm.aggregate_reward_in_turn,
                            thought_prob_coef=self.config.algorithm.get("thought_prob_coef", 1.0),
                        )
                    # update critic
                    if self.use_critic:
