...
from hygae.env.utils.env_utils import permanent_seed
...
def create_dataset_from_yaml(yaml_file_path: str, force_gen=False,seed=42,train_path='./train.parquet',test_path='./test.parquet'):
    ...
    if isinstance(yaml_file_path, (str, os.PathLike)):
        with open(yaml_file_path, 'r') as f:
            yaml_config = yaml.safe_load(f)
    else:
        yaml_config = yaml_file_path

    yaml_config = _expand_environment_variables(yaml_config)
    ...
    for key, value in yaml_config.items():
        ...
        if hasattr(env_config, 'generate_seeds'):
            ...
            print(f"Using {len(seeds_for_env_train)} train seeds generated by {env_name} config's generate_seeds method")
