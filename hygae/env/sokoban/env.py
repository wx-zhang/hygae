...
from hygae.env.utils.context_utils import convert_numpy_to_PIL
from hygae.env.utils.env_utils import NoLoggerWarnings, set_seed
from hygae.env.utils.parse_utils import PARSE_FUNC_MAP
from hygae.env.utils.state_reward_text_utils import env_state_reward_wrapper
...
from .utils import (
    convert_sokoban_state_to_relative_list,
    generate_room,
)

_VISUAL_PROMPT_IMAGE_PATH = os.path.join(os.path.dirname(__file__), "image.png")
...
class SokobanEnv(BaseEnv):
    ...
    def system_prompt(self):
        format_prompt = self.format_prompt_func(
            ...
            visual_prompt=self.config.visual_prompt,
    ...
    def _render(self, init_obs=True):
        ...
        if self.config.render_mode == "vision":
            ...
            multi_modal_data = {
                img_placeholder: [
                    convert_numpy_to_PIL(self.env.render(mode="rgb_array"))
                ],
            }
        ...
        if init_obs:
            ...
            obs_str = init_observation_template(img_str=img_str) + "\n"
            # Insert example game image into multi_modal_data when visual_prompt is enabled
            # The <image> placeholder is already in the system prompt (via format_prompt with add_example=True)
            if self.config.visual_prompt and self.config.render_mode == "vision":
                example_img = Image.open(_VISUAL_PROMPT_IMAGE_PATH).convert("RGB")
                multi_modal_data[img_placeholder].insert(0, example_img)
        else:
            obs_str = (
                action_template(
    ...
    def get_env_state(self):
        ...
        player_pos = tuple(
            map(int, np.argwhere(np.logical_or(room_state == 5, room_state == 6))[0])
        )

        # Find box positions (codes 3: box on target, 4: box not on target)
        box_positions = [
            tuple(map(int, pos))
            for pos in np.argwhere(np.logical_or(room_state == 3, room_state == 4))
        ]
        ...
        target_positions = [
            tuple(map(int, pos)) for pos in np.argwhere(self.env.room_fixed == 2)
        ]
