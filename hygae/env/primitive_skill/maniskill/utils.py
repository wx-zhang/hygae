...
from hygae.env.primitive_skill.maniskill.skill_wrapper import SkillGymWrapper
import numpy as np
import os
...
def build_env(env_id, control_mode="pd_ee_delta_pose", stage=0, record_dir='./test'):
    render_backend = os.environ.get("HYGAE_PRIMITIVE_RENDER_BACKEND", "gpu")
    if render_backend not in {"cpu", "gpu"}:
        raise ValueError(
            "HYGAE_PRIMITIVE_RENDER_BACKEND must be 'cpu' or 'gpu', "
